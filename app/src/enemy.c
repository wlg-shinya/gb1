...
void enemy_collision(uint8_t i)
{
  uint8_t j;
  for( j=0; j<MAX_TT; j++ ) {
    if( tf[j] != 0 ) {
  ...
  if( ef[i] == 1 ) {
    ...
    if( make_rnd(rnd_kirai) == 0 ) {
      if( ((ex[i]-SUB_EX0)>MIN_PX)&&((ex[i]-SUB_EX0)<MAX_PX) ) {
        if( kf[i] == 0 ) {
          kf[i] = 1; kx[i] = ex[i]-SUB_EX0+4; ky[i] = ey[i]-4;
        } else if( kf[i+1] == 0 ) {
          kf[i+1] = 1; kx[i+1] = ex[i]-SUB_EX0+4; ky[i+1] = ey[i]-4;
        } else if( kf[i+2] == 0 ) {
          kf[i+2] = 1; kx[i+2] = ex[i]-SUB_EX0+4; ky[i+2] = ey[i]-4;
        }
      }
    }
  } else if( ef[i] == 2 ) {
